    ...
    <div class="my-history container mt-5">
      ...
      <main>
        <div class="history-list">
          <div class="card mb-4 shadow-sm" v-for="history in histories" :key="history.id">
            <div class="card-body">
              <h5 class="card-title">{{ history.date }} with {{ history.psychologist }}</h5>
              <p class="card-text">
                Time: {{ history.service }}<br />
                Cost: {{ history.cost }}
              </p>
            </div>
          </div>
